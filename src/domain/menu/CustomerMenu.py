...
class CustomerMenu(Menu):
    ...
    def add_customer_by_xl(self):
        ...
        while True:
            customer_id_row = input()
            if Validator.validate_is_go_back_menu(customer_id_row):
                return
            if Validator.validate_row(sheet, customer_id_row):
                break
            else:
                print("잘못된 입력입니다.")
        print("읽어들일 고객 이름의 열을 입력해주세요. (예 : B)")
        while True:
            customer_name_row = input()
            if Validator.validate_is_go_back_menu(customer_name_row):
                return
            if Validator.validate_row(sheet, customer_name_row):
                break
            else:
                print("잘못된 입력입니다.")
        print("읽어들일 고객 이용 금액의 열을 입력해주세요. (예 : C)")
        while True:
            customer_price_row = input()
            if Validator.validate_is_go_back_menu(customer_price_row):
                return
            if Validator.validate_row(sheet, customer_price_row):
                break
            else:
                print("잘못된 입력입니다.")
        print("읽어들일 고객 이용 시간의 열을 입력해주세요. (예 : D)")
        while True:
            customer_time_row = input()
            if Validator.validate_is_go_back_menu(customer_time_row):
                return
            if Validator.validate_row(sheet, customer_time_row):
                break
            else:
                print("잘못된 입력입니다.")
        ...
        while True:
            try:
                customer_start_col = int(input())
                if Validator.validate_is_go_back_menu(customer_start_col):
                    return
                if Validator.validate_col(sheet, customer_start_col, Validator.validate_convert_row_to_int(customer_id_row), Validator.validate_convert_row_to_int(customer_name_row),
                                          Validator.validate_convert_row_to_int(customer_price_row),
                                          Validator.validate_convert_row_to_int(customer_time_row)):
                    break
                else:
                    print("잘못된 입력이거나 데이터가 존재하지 않는 행/열 입니다.")
            except ValueError:
                print("숫자만 입력해주세요.")
        print("검증에 실패한 데이터는 입력되지 않습니다.")
        print("고객 정보를 입력합니다.")
        print(f"고객 아이디 열 : {customer_id_row}, 규칙: {Validator.regex_explain_id}")
        print(f"고객 이름 열 : {customer_name_row}, 규칙: {Validator.regex_explain_name}")
        print(f"고객 이용 금액 열 : {customer_price_row}, 규칙: 0 이상의 정수, 0일 시 입력되지 않음")
        print(f"고객 이용 시간 열 : {customer_time_row}, 규칙: 0 이상의 정수, 0일 시 입력되지 않음")
        print(f"고객 정보 시작 행 : {customer_start_col}")
        ...
        if yes_or_no == "n":
            ...
            return
        print("정보를 입력합니다.")
        self.add_customer_by_xl_work(sheet, customer_id_row, customer_name_row, customer_price_row, customer_time_row,
                                     customer_start_col)
        print("모든 정보를 입력했습니다.")

    def add_customer_by_xl_work(self, sheet, customer_id_row, customer_name_row, customer_price_row, customer_time_row,
                                customer_start_col):
        idx_num = 1
        for i in range(customer_start_col, sheet.max_row + 1):
            customer_id = sheet[f"{customer_id_row}{i}"].value
            customer_name = sheet[f"{customer_name_row}{i}"].value
            customer_price = sheet[f"{customer_price_row}{i}"].value
            customer_time = sheet[f"{customer_time_row}{i}"].value
